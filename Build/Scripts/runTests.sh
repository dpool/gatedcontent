...
read -r -d '' HELP <<EOF
gatedcontent test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/gatedcontent/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="Web/typo3conf/ext/gatedcontent/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="Web/typo3conf/ext/gatedcontent/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/gatedcontent/Tests/Unit"
            ;;
    esac
fi
...
case ${TEST_SUITE} in
    acceptance)
        setUpDockerComposeDotEnv
        docker-compose run acceptance_frontend_mariadb10
        SUITE_EXIT_CODE=$?
        docker-compose down
        ;;
    composerInstall)
